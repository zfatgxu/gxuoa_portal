<template>
  <div>
    ...
    <div class="sidebar">
      <ul class="sidebar-menu">
        <li :class="{ open: isWorkbenchOpen }">
          ...
          <ul class="submenu">
            <li><router-link to="/home/workplace/mytodo" :class="{ active: $route.path === '/home/workplace/mytodo' }">我的待办</router-link></li>
            <li><router-link to="/home/workplace/myisdone" :class="{ active: $route.path === '/home/workplace/myisdone' }">我的已办</router-link></li>
            <li><router-link to="/home/workplace/myall" :class="{ active: $route.path === '/home/workplace/myall' }">全部</router-link></li>
            <li><router-link to="/home/workplace/myapply" :class="{ active: $route.path === '/home/workplace/myapply' }">我的申请</router-link></li>
          </ul>
        </li>
        <li>
          <a href="#" id="doc-menu" @click.prevent="toggleDocMenu">
    ...
    <div class="main-content">      
      <!-- 使用router-view替代原来的v-if/v-show控制的内容区 -->
...
const goToTodoList = () => {
  // 使用路由导航替代直接修改变量
  router.push('/home/workplace/mytodo')
}

// 切换公文流转菜单展开状态
const toggleDocMenu = () => {
  isDocMenuOpen.value = !isDocMenuOpen.value
  console.log('公文流转菜单展开状态：', isDocMenuOpen.value)
  
  // 更新箭头方向
  const docMenu = document.getElementById('doc-menu')
  const arrow = docMenu?.querySelector('.arrow i')
  if (arrow) {
    if (isDocMenuOpen.value) {
  ...
  // 更新菜单项的open类
  const menuItem = docMenu?.parentElement
  if (menuItem) {
    if (isDocMenuOpen.value) {
...
const listenToComponentSwitch = () => {
  ...
  eventBus.on(EVENT_NAMES.SWITCH_COMPONENT, (componentName: string) => {
    ...
    // 使用路由导航替代直接修改变量
    router.push(`/${componentName.replace('Component', '').toLowerCase()}`)
  })
