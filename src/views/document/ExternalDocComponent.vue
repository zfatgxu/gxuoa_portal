<template>
  <div class="content-section">
    ...
    <div class="content-body">
      ...
      <div v-if="!loading && externalDocs.length > 0">
        <el-table :data="externalDocs" style="width: 100%" border stripe>
...
const currentComponent = ref('ExternalDocComponent')
...
const handleProcess = (row: any) => {
  ...
  const relativePath = `/documentApproval?id=${row.id}&type=1`
  ...
  try {
    ...
    if (!newWindow) {
      ...
      // 备选方案：在当前窗口打开
      router.push({ name: 'documentApproval', query: { id: row.id, type: 1 } })
    }
  } catch (error) {
