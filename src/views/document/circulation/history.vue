...
import { CirculationApi } from '@/api/document/circulation/index'
...
const getList = async () => {
  try {
    ...
    const res = await CirculationApi.getCirculationList(Number(documentId))
    if (res && res.data && res.data.length) {
      circulationList.value = res.data
      
      // 根据流转记录生成流程步骤
      generateProcessSteps(res.data)
    } else {
