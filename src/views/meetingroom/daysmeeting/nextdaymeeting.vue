<template>
  <div class="nextday-meeting-container">
    <div class="page-header">
      <h2>明日会议</h2>
      <div class="date-display">{{ formattedNextday }}</div>
    </div>

    <!-- 会议列表 -->
    <div class="meeting-list-container">
      <div v-if="nextdayMeetings.length > 0">
        <el-table :data="nextdayMeetings" border style="width: 100%">
...
// 明日会议列表
const nextdayMeetings = ref([])

// 格式化的明日日期
const formattedNextday = computed(() => {
  return dayjs().add(1, 'day').format('YYYY年MM月DD日 dddd')
})

// 获取明日会议数据
const loadNextdayMeetings = async () => {
  try {
    // 获取明天的日期
    ...
    nextdayMeetings.value = res.list
      .filter(item => {
        // 过滤已通过的会议
        ...
        return startDate === tomorrowDateStr;
      })
      .map(item => ({
        index: item.id,
    ...
    console.error('加载明日会议失败:', error)
    ElMessage.error('加载明日会议失败')
  }
}
