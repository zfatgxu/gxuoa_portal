...
const saveForm = async () => {
      ...
      // 将表单数据更新
      await InfoApi.updateInfo(formData)
...
const getDataDetail = async () => {
  ...
  if (res) {
    // 处理协办单位列表
    if (res.cooperationUnits) {
      cooperationUnits.value = res.cooperationUnits.split(',').map(unit => unit.trim())
    }
    // 处理时间格式
    if (res.requiredCompletionTime) {
      formData.requiredCompletionTime = dayjs(res.requiredCompletionTime).format('YYYY-MM-DD HH:mm:ss')
    }
    // 处理信访附件列表
    if (res.petitionList) {
      petitionList.value = res.petitionList.map(file => {
        return {
          name: file.name,
          size: file.size,
          url: file.url
    ...
    // 处理处理报告附件列表
    if (res.petitionHandlingFileList) {
      petitionHandlingFileList.value = res.petitionHandlingFileList.map(file => {
        return {
          name: file.name,
          size: file.size,
          url: file.url
    ...
    // 处理答复意见附件列表
    if (res.petitionReplyFileList) {
      petitionReplyFileList.value = res.petitionReplyFileList.map(file => {
        return {
          name: file.name,
          size: file.size,
          url: file.url
